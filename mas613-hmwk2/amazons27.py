...
# * time_limit -- # of seconds a machine is allowed to take (<30)
...
firstmove = True
curPos = [] # each index represents position of each queen
...
def getQueenPos(config,white=True):
    ch = 'Q'
    if not white:
        ch='q'
    ...
    for r in config:
        for c in r:
            if c==ch:
                curPos.append((config.index(r),r.index(c)))
       
def heuristic():


def getChildStates(board,srcr,srcc):
    dim = len(board)
    dstlist = []
    ### Horizontal Moves ###
    ...
    i=srcr-1
    while i>=0:
        if board[i][srcc]!='x' and board[i][srcc]!='Q' and board[i][srcc]!='q':
            dstlist.append((i,srcc))
            i-=1
        else:
            break
    ...
    i=srcc-1
    while i>=0:
        if board[srcr][i]!='x' and board[srcr][i]!='Q' and board[srcr][i]!='q':
            dstlist.append((srcr,i))
            i-=1
        else:
            break
    ...
    j=srcc+1
    while i>=0 and j<dim:
        if board[i][j]!='x' and board[i][j]!='Q' and board[i][j]!='q':
            dstlist.append((i,j))
            i-=1
            j+=1
        else:
...
def mas613(board):
    ...
    if board.bWhite:
        print("White Player turn (Q)")
    else:
        print("Black Player turn (q)")
